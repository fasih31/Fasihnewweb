...
export function BNPLCalculator() {
  ...
  return (
    <CalculatorCard
      title="Islamic BNPL Calculator"
      description="Murabaha-based Buy Now, Pay Later"
      icon={ShoppingBag}
      gradientFrom="from-emerald-50"
      gradientTo="to-teal-50 dark:from-emerald-950 dark:to-teal-950"
      borderColor="border-emerald-200 dark:border-emerald-800"
    >
      <div className="space-y-4 sm:space-y-6">
        <div className="space-y-2">
          <Label className="text-sm sm:text-base">Product Price (AED)</Label>
          <Input
            type="number"
            value={productPrice}
        ...
        <div className="space-y-2">
          <Label className="text-sm sm:text-base">Number of Installments: {installments} months</Label>
          <Slider
            value={[installments]}
        ...
        <div className="space-y-2">
          <Label className="text-sm sm:text-base">Markup Percentage: {markupPercent}%</Label>
          <Slider
            value={[markupPercent]}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          ...
          <Card className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950 dark:to-indigo-950">
            <CardContent className="p-3 sm:p-4">
              <p className="text-xs sm:text-sm text-muted-foreground mb-1">Total Cost</p>
          ...
          <Card className="bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-950 dark:to-pink-950">
            <CardContent className="p-3 sm:p-4">
              <p className="text-xs sm:text-sm text-muted-foreground mb-1">Total Markup</p>
        ...
        <div className="p-3 sm:p-4 bg-muted rounded-lg space-y-2">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h4 className="font-semibold text-sm sm:text-base">Payment Breakdown</h4>
          </div>
          <div className="max-h-48 overflow-y-auto space-y-2">
            {Array.from({ length: installments }, (_, i) => (
              <div key={i} className="flex justify-between items-center py-2 border-b last:border-0">
                <span className="text-xs sm:text-sm">Month {i + 1}</span>
                <Badge variant="outline" className="text-xs sm:text-sm">AED {monthlyPayment.toFixed(2)}</Badge>
              </div>
            ))}
          </div>
        </div>

        <div className="p-3 sm:p-4 bg-emerald-50 dark:bg-emerald-950 rounded-lg border border-emerald-200 dark:border-emerald-800">
